...
Read `.relay/state.json` before doing anything.
- If `mode` is `"inspect"` or `"audit"` → skip Mode A and Mode B. Run **Mode C** below.
- Otherwise → follow Mode A and Mode B instructions above (unchanged).

---

## Mode C: Existing Solution Security Audit

Invoked when `mode: inspect` or `mode: audit`. No `plan.md` or `security-design.md` required.

**Inputs:** `docs/existing-solution.md` + `docs/project-context.md` (may be inferred).

### Step C1 — Infer Intended Security Model

Read `docs/project-context.md`.

- If `context_mode: provided` → use the stated purpose and personas to define what security *should* look like
- If `context_mode: inferred` → infer from solution structure:
  - Table names + relationship patterns suggest the domain (e.g., leave system, asset tracker, training portal)
  - Flow triggers + actions suggest who does what
  - Security role names suggest intended personas

Document your inferred model:
```
Apparent purpose: <1 sentence>
Inferred personas: <list>
Expected privilege pattern: <e.g., "Staff: User-scoped read/write own records. Manager: BU-scoped read. Admin: Org-scoped full.">
```

### Step C2 — Security Checks (same 8 dimensions as Mode A, applied to discovered reality)

1. **Ownership model** — Are tables that need row-level security set to User-owned? Flag Org-owned tables where user-scoped data isolation appears required.

2. **Role privilege scope** — For each custom role: does the privilege scope match the inferred persona's need? Flag Org-level grants where BU-level would suffice.

3. **FLS coverage** — Identify columns that appear sensitive (financial amounts, personal data, approval decisions, passwords, API keys). Are FLS profiles configured for them? Verify FLS is enforced at Web API layer (not just UI).

4. **UI-vs-actual-security traps** — Scan for:
   - JavaScript `setVisible(false)` / `setDisabled(true)` used as the only security on a field
   - Business Rules that hide/show fields (UI only)
   - Sitemap entries removed but table still accessible via direct URL
   - Canvas `Filter()` without table permission enforcement
   - View filters presented as access control

5. **Connection reference identity** — For each flow: is the connection using maker identity? If the maker has higher privileges than the flow's intended invoking user, this is a privilege escalation path.

6. **DLP compliance** — List connectors in use. Flag any Business + Non-Business tier mixing. Flag premium connectors that may require licensing review.

7. **Over-privileged roles** — Flag any role with Org-level Write/Delete on tables that appear to be user-scoped data.

8. **Missing roles** — Based on inferred personas, are there any roles that should exist but don't? Flag absent personas.

### Step C3 — Output

Write findings to `docs/audit-report.md` under a `## Security Findings` section:

```markdown
## Security Findings

### Inferred Security Model
Apparent purpose: <...>
Inferred personas: <...>
Expected privilege pattern: <...>

### Security Checks

| # | Check | Result | Finding |
|---|---|---|---|
| C1 | Ownership model appropriate | ✅ / ❌ | <detail> |
| C2 | Role privilege scopes minimal | ✅ / ❌ | <detail> |
| C3 | FLS on sensitive columns | ✅ / ❌ | <detail> |
| C4 | No UI-only security traps | ✅ / ❌ | <detail> |
| C5 | No maker-identity escalation | ✅ / ❌ | <detail> |
| C6 | DLP compliant | ✅ / ❌ | <detail> |
| C7 | No over-privileged roles | ✅ / ❌ | <detail> |
| C8 | All expected roles present | ✅ / ❌ | <detail> |

### Critical Security Findings
<detailed findings for any ❌ items>
```

Return to Conductor:
```
Security checks: <N> passed, <N> failed
