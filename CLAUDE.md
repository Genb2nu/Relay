...
Phase 2 — PLANNING
  ...
  Phase 2 Planning Substep — Wireframes
    ...
    3. Wait for user reply: approved or change requests
    4. If changes requested → Stylist updates docs/wireframes.html → re-confirm (max 1 revision)
    5. Once approved → Conductor sets phase2_planning.wireframes_approved = true
    6. Proceed with Phase 2 scoring and gate validation
  Run: python scripts/relay-score.py
  Run: python scripts/relay-gate-check.py --phase 2  (includes consistency check)
  Show plan summary + scores to user → proceed to Phase 3 when gate passes

Phase 3 — PLAN REVIEW (Auditor + Warden parallel)
  Invoke Auditor → reviews plan.md + docs/wireframes.html for completeness
    Auditor checks: screen count matches plan.md and user story flows map to wireframed screens
  Invoke Warden → reviews plan.md + security-design.md + docs/wireframes.html for security
    Warden checks: approval/sensitive actions are hidden from unauthorised personas and wireframe visibility matches security-design.md
...
```
/fleet
@auditor.agent Read docs/plan.md, docs/requirements.md, and docs/wireframes.html. Review plan completeness. Confirm screen count and user story flows are represented in the wireframes. Write approval status and issues to .relay/plan-index.json phase3_review.auditor_approved.
@warden.agent Read docs/plan.md, docs/security-design.md, and docs/wireframes.html. Read skills/power-platform-security-patterns/SKILL.md. Review security design and persona visibility in the wireframes. Write approval status and issues to .relay/plan-index.json phase3_review.warden_approved.
```
