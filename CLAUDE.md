...
When running from Copilot CLI (not VS Code), use /fleet at every parallel phase.
Subagents do NOT inherit chat history — every /fleet prompt must be self-contained,
pointing agents at the files they need to read.

### Phase 3 /fleet prompt
```
/fleet
@auditor.agent Read docs/plan.md and docs/requirements.md. Review plan for completeness. Write approval status to .relay/plan-index.json phase3_review.auditor_approved.
@warden.agent Read docs/plan.md and docs/security-design.md. Read skills/power-platform-security-patterns/SKILL.md. Review security. Write approval to .relay/plan-index.json phase3_review.warden_approved.
```

### Phase 5a /fleet prompt
```
/fleet
@vault.agent Read docs/plan.md and .relay/state.json for prefix. Build Dataverse schema, roles, FLS, env vars. Write GUIDs to .relay/plan-index.json components.
@stylist.agent Read docs/plan.md and docs/requirements.md. Read skills/canvas-app-design-reading/SKILL.md. Produce docs/design-system.md.
```

### Phase 5b /fleet prompt
```
/fleet
@forge.agent Read docs/plan.md, docs/design-system.md, .relay/state.json. Build Canvas App via Canvas Authoring MCP. Output: src/canvas-apps/*.pa.yaml
@forge.agent Read docs/plan.md, .relay/state.json. Build MDA sitemap XML via Dataverse API. Read skills/power-platform-alm/SKILL.md. Do NOT use /genpage for standard MDA.
@forge.agent Read docs/plan.md, .relay/state.json. Generate flow JSON + import via pac solution import + activate via clientdata PATCH. Read skills/power-platform-alm/SKILL.md.
```

### Phase 5b inline verification /fleet prompt
```
/fleet
@sentinel.agent Verify Canvas App: run App Checker (all 5 categories). Read docs/plan.md.
@sentinel.agent Verify MDA: sitemap areas, forms, views, published. Read docs/plan.md.
@sentinel.agent Verify Flows: all imported, all active (statecode=1), connections linked.
```

### Phase 6 /fleet prompt
```
/fleet
@sentinel.agent Run scripts/relay-drift-check.py. Generate and run Playwright tests from docs/requirements.md. Read skills/playwright-testing/SKILL.md. Write results to .relay/plan-index.json phase6_verify.sentinel_approved.
@warden.agent Execute scripts/security-tests.ps1. Verify FLS, role boundaries, self-approval prevention. Write results to .relay/plan-index.json phase6_verify.warden_approved.
```

### /relay:audit /fleet prompt (after Analyst completes)
```
/fleet
@auditor.agent Read docs/existing-solution.md. Review completeness and best practices.
@warden.agent Read docs/existing-solution.md. Read skills/power-platform-security-patterns/SKILL.md. Review security gaps.
@critic.agent Read docs/existing-solution.md. Read skills/power-platform-footgun-checklist/SKILL.md. Run full checklist.
```

### /relay:change review /fleet prompt
```
/fleet
@auditor.agent Read docs/change-plan.md + docs/existing-solution.md. Does change break existing?
@warden.agent Read docs/change-plan.md + docs/existing-solution.md. Does change introduce security gaps?
```

### /relay:visualise /fleet prompt
```
/fleet
Generate Mermaid diagrams from docs/requirements.md — persona map + user story map
Generate Mermaid diagrams from docs/plan.md — ERD + architecture + flow diagrams + gantt
Generate Mermaid diagrams from docs/security-design.md — permission matrix + data flow
```

### Cost note
Each /fleet subagent consumes its own premium requests. For Phase 5b with 3 Forge
agents, one fleet = ~3-9 premium requests depending on model. Use /fleet when
parallelism saves meaningful time. Sequential mode is simpler and cheaper.

### Availability
/fleet is Copilot CLI only — not available in VS Code Agent Mode.
In VS Code, Conductor runs all agents sequentially with identical output quality.

---

## /fleet Parallel Execution (Copilot CLI only)

When running from Copilot CLI, use /fleet at every phase where agents are independent.
Subagents do NOT inherit chat history — every /fleet prompt must be self-contained,
pointing agents at the files they need to read.
...
```
/fleet
@auditor.agent Read docs/plan.md and docs/requirements.md. Review plan completeness. Write approval to .relay/plan-index.json phase3_review.auditor_approved.
@warden.agent Read docs/plan.md and docs/security-design.md. Read skills/power-platform-security-patterns/SKILL.md. Review security. Write approval to .relay/plan-index.json phase3_review.warden_approved.
```
...
```
/fleet
@vault.agent Read docs/plan.md and .relay/state.json. Build Dataverse schema. Write GUIDs to .relay/plan-index.json components.
@stylist.agent Read docs/plan.md and docs/requirements.md. Produce docs/design-system.md.
```

### Phase 5b — Apps + Flows
```
/fleet
@forge.agent Read docs/plan.md, docs/design-system.md, .relay/state.json. Build Canvas App via Canvas Authoring MCP.
@forge.agent Read docs/plan.md, .relay/state.json. Build MDA sitemap XML via Dataverse API. Read skills/power-platform-alm/SKILL.md.
@forge.agent Read docs/plan.md, .relay/state.json. Generate flow JSON + import + activate via clientdata PATCH. Read skills/power-platform-alm/SKILL.md.
```

### Phase 5b — Inline Verification
```
/fleet
@sentinel.agent Verify Canvas App: App Checker all 5 categories (Formulas, Runtime, Accessibility, Performance, Data source).
@sentinel.agent Verify MDA: sitemap areas, forms, views, publish status.
@sentinel.agent Verify Flows: all imported, all active, connection refs linked.
```

### Phase 6 — Final Verification
```
/fleet
@sentinel.agent Read docs/plan.md and .relay/plan-index.json. Run drift detection. Generate and run Playwright tests. Write to plan-index.json phase6_verify.sentinel_approved.
@warden.agent Read docs/security-design.md. Execute scripts/security-tests.ps1. Write to plan-index.json phase6_verify.warden_approved.
```

### /relay:audit — After Analyst
```
/fleet
@auditor.agent Read docs/existing-solution.md. Review completeness + best practices.
@warden.agent Read docs/existing-solution.md. Read skills/power-platform-security-patterns/SKILL.md. Review security gaps.
@critic.agent Read docs/existing-solution.md. Read skills/power-platform-footgun-checklist/SKILL.md. Run full checklist.
```

### /relay:change + /relay:visualise
```
# /relay:change review
/fleet
@auditor.agent Read docs/change-plan.md. Check change doesn't break existing.
@warden.agent Read docs/change-plan.md. Check change doesn't introduce security gaps.

# /relay:visualise
/fleet
Generate Mermaid from docs/requirements.md — persona map + user story map
Generate Mermaid from docs/plan.md — ERD + architecture + flow diagrams + Gantt
Generate Mermaid from docs/security-design.md — permission matrix
```

### Cost warning
Each /fleet subagent consumes its own premium requests. Use /fleet when parallelism
saves meaningful time. For single-component builds, sequential mode is cheaper.
/fleet is exclusive to Copilot CLI — not available in VS Code Agent Mode.
...
Step 8: All pass → Phase 6 gate clears → proceed to Phase 7 (ship)
```
