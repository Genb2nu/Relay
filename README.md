...
YOU ──► Conductor
       ...
       Scout ──────────► docs/requirements.md + state updates
        ...
        Drafter ─────────► docs/plan.md + docs/security-design.md
           ...
           │
       Stylist (Mode C) ─► docs/wireframes.html (presentation-friendly preview, not a bare skeleton)
           │
       ...
       Stylist ─┘
           │
       Forge-Canvas ───────► Checklist A (solution-scoped create + save + coauthoring + stable Data pane) → Canvas Authoring MCP → src/canvas-apps/*.pa.yaml
...
> If a build-phase specialist cannot execute directly in the current interface but returns exact scripts or artifacts, Conductor should persist and run them instead of treating the phase as failed immediately.
...
/fleet
  @sentinel.agent ──► maker/browser preflight for Power Pages + flow-trigger availability
...
- **Phase 2:** `docs/plan.md`, `docs/security-design.md`, and `docs/wireframes.html` must exist, wireframes must be approved, and the plan must be explicit enough for Vault to build without inventing schema details
...
|---|---|
| `.relay/plan-index.json` | Machine-readable contract — phase gate status, component inventory, quality scores, optional built-component IDs |
| `.relay/state.json` | Authoritative project state plus shared component GUID maps for build coordination |
...
| `tests/test-gates.py` | Regression tests for Phase 1, Phase 2 build-readiness, and Phase 5 gate enforcement |
...
| `playwright-testing` | Playwright E2E for Canvas + MDA with Page Object Model plus maker-surface auth/blocker guidance |
| `relay-recovery-playbook` | Recovery sequences for Power Pages, flow-trigger, and MDA deployment blockers |
...
### v0.7.0 (current)
- **Planning and build gates hardened** — discovery now requires a real `docs/requirements.md`, planning must prove `build_ready_for_vault`, and specialist build flags only pass when actual Canvas, MDA, and Power Pages artifacts exist
- **Forge continuation made explicit** — Relay now consistently routes Phase 5 from Vault + Stylist into Forge-Canvas, Forge-MDA, Forge-Flow, Forge-Pages, and Forge with solution-linked completion checks
- **Canvas and UX quality guidance improved** — Stylist now produces presentation-friendly HTML wireframes, review mode treats wrap/clipping/sizing defects as major, and Forge-Canvas enforces the safer maker bootstrap sequence
- **Recovery and browser automation strengthened** — Relay now includes the `relay-recovery-playbook` skill plus per-surface maker login verification, maker preflight checks, and transient blocker clearing guidance in `playwright-testing`
- **Vault and Dataverse metadata guidance tightened** — solution headers, component-count verification, safer schema payloads, and consistent privilege-depth/action guidance are now documented and enforced more clearly
- **Regression coverage expanded** — `tests/test-gates.py` and consistency checks now cover the build-readiness and artifact-enforcement gaps that previously slipped through
